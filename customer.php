...
<div class='mid'>
<center>
<form id="cfm" action="customer.php" method="post">
<h3>Enter M-PESA transacion ID to allow booking.</h3>
<table>
<tr>
<td><strong>ENTER M-PESA TRANSACTION ID:</strong></td>
<td><input class="input" type="text" name="TID" value="" required="required"></td>
</tr>
<tr>
<td><strong>ENTER PHONE NO:</strong></td>
<td><input class="input" type="text" name="phone" value="" required="required"></td>
</tr>
<tr>
<td></td>
...
$query = "select TID from mpesa where TID=" . $_POST['TID'];
$result = mysql_query($query) or die("Error in the query: $query");
 if (mysql_num_rows($result) == 0) {
   echo("Sorry invalid IDNO");
   
 }else {
    $getquery=mysql_query("SELECT*FROM booking where phone=". $_POST['phone']);
 ...
 echo "</table>";
   ...
   echo "<input type=\"button\" value=\"Print receipt\" onclick=\"printtable()\" />";
